...
export default class Gameselected extends Component {
	...
	handleBetRisk = (e, item) => {
		...
		if (item.wager_juice < 0) {
			...
			let calcBet = bet => {
				let newAmount = item.wager_juice * 0.01 * bet;
				return newAmount;
			};
			let win = calcBet(bet);
			win = Math.round(win * 100) / 100;
					...
					<div className="game-selected-container" key={i}>
						...
						<div className="bet-container">
							<div className="bet-chosen">
								<span className="teamChozen">
									{item.wager_teamChosen}{' '}
									{(item.wager_line > 0 && item.wager_type === 'moneyline') ||
									(item.line > 0 && item.wager_type === 'spread')
										? '+ ' + item.wager_line
										: item.wager_line}{' '}
								</span>
								<span className="wager-type">{item.wager_type}</span>
							</div>
							<span className="wager-type">
						...
						<div className="match-container">
							...
							<span className="match-time">
								{this.props.convertTime(item.wager_matchTime)}
							</span>
						</div>
						<div className="wager-amount">
							<div className="group-risk">
								<input
									type="number"
									name="risk"
									value={item.wager_risk}
									onChange={e => this.handleBetRisk(e, item)}
								/>
								<span>Risk</span>
							</div>
			...
			<div id="game-selected">
				...
				<div className="wager-confirmation-container">
					...
					<div className="confirmation-group bet-risk">
						...
						<h5>
							{' '}
							{isNaN(this.totalMoneyRisked())
								? ' Fill-in bet'
								: '$' + this.totalMoneyRisked()}
						</h5>
					</div>
					<div className="confirmation-group bet-win">
						...
						<h5>
							{' '}
							{isNaN(this.totalMoneyWinPossible())
								? ''
								: '$' + this.totalMoneyWinPossible()}
						</h5>
					</div>
					<button type="text" onClick={this.props.submitForm}>
						Place Wager
					</button>
				</div>
			</div>
